...
1. **Flash firmware** to your ESP32 using [ESPHome Web](https://web.esphome.io/)
2. **Connect to WiFi**: The device creates a WiFi hotspot `bfour-bf-60` (password: `bfour-bf60`). Connect and configure your WiFi credentials via the captive portal.
3. **Adopt the device** in your ESPHome Dashboard
4. **Discover your BF-60 MAC address**:
   - Make sure your BF-60 base station is powered on
   - Close the BFOUR app on your phone (so it's not connected)
   - In Home Assistant, press the **"Discover BF-60"** button
   - Within 30 seconds, check the **"Discovered BF-60 MAC"** sensor for your MAC address
   - Or check the ESPHome logs for `[DISCOVERY] MAC: XX:XX:XX:XX:XX:XX`
5. **Configure the MAC address** in ESPHome Dashboard:
   - Click on the device → Edit
   - Add this at the top of your device config:
     ```yaml
     substitutions:
       grill_mac: "XX:XX:XX:XX:XX:XX"  # Replace with YOUR discovered MAC
     ```
   - Save and Install
...
The BF-60 base station advertises as **"Grill BT5.0"** over Bluetooth.

**Method 1: Built-in Discovery (Recommended)**
1. Press the **"Discover BF-60"** button in Home Assistant
2. Power on your BF-60 base station (close the BFOUR app first)
3. The MAC appears in the **"Discovered BF-60 MAC"** sensor within 30 seconds

**Method 2: nRF Connect App**
1. Install nRF Connect on your phone
2. Scan for BLE devices
3. Look for "Grill BT5.0" - the MAC is shown next to the name
...
## Home Assistant Cooking Assistant Package

An optional Home Assistant package adds smart cooking workflow features:

### Features
- **Start/Stop Cooking** - Track cooking sessions with automatic timers
- **USDA Temperature Presets** - Quick selection of safe cooking temperatures
- **Custom Presets** - Save your favorite meat/temperature combinations
- **Turn Meat Alert** - Notification when meat reaches halfway point
- **Remove Meat Alert** - Notification when target temperature is reached
- **Rest Timer** - Countdown timer for resting meat after cooking
- **Rest Complete Alert** - Notification when meat has finished resting

### Installation

1. Copy `homeassistant/bfour_bf60_package.yaml` to your HA `config/packages/` directory
2. Enable packages in `configuration.yaml`:
   ```yaml
   homeassistant:
     packages: !include_dir_named packages/
   ```
3. **Important:** Edit the package file and replace all instances of `bfour_bf_60_XXXXXX` with your actual ESPHome device name
...
| Beef - Medium | 60°C / 140°F |
| Beef - Medium Well | 65°C / 150°F |
